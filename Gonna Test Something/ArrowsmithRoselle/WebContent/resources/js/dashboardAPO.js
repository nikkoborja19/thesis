...
function deleteTemporaryOffering(){
	...
	while(tempCourseOfferings[i].courseId !== concatId){
		console.log("tempShitID: " + tempCourseOfferings[i].courseId);
		i++;
	}
	...
	if(tempCourseOfferings[i].courseId === concatId){
		tempCourseOfferings.splice(i, 1); //delete 1 element starting from index <deleteIndex>
		updateTemporaryOfferingList();
	}
...
$('#textStartYearANO').on('change', function(e){
	var endYear = parseInt($('#textStartYearANO').val()) + 1;
	$('#textEndYearANO').val(endYear);
})

$('#textStartYearANO').on('keyup', function(e){
	var endYear = parseInt($('#textStartYearANO').val()) + 1;
	$('#textEndYearANO').val(endYear);
})
/****EVENT LISTENERS FOR SEARCH****/
